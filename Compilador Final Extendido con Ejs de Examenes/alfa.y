...
int num_potencias = 0; /* Indica numero de potencia en codigo */
int num_incrementos = 0; /* Indica numero de incremento en codigo */
int num_inits = 0; /* Indica numero de init en codigo */
int num_init = 0; /* Indica numero de expresiones en regla init */
%}
/******************************************/
...
%left TOK_ASTERISCO TOK_DIVISION TOK_AND TOK_MODULO
%right TOK_POTENCIA
...
/* Examen 2014 */
%type <atributos> compare_with_exp
%type <atributos> less_sentencias
%type <atributos> equal_sentencias
%type <atributos> greater_sentencias
/***************/
...
sentencia_simple: asignacion {
	...
	/* Examen 2015 */
	| init_vector {
		fprintf(yyout,";R38:\t<sentencia_simple> ::= <init_vector>\n");
	}
	/***************/
;
...
asignacion: TOK_IDENTIFICADOR TOK_ASIGNACION exp {
	...
	/* Examen 2013 */
	| TOK_IDENTIFICADOR TOK_INCREMENTO exp {
		lectura = UsoLocal($1.lexema);
		if(lectura == NULL){
			fprintf(yyout,"****Error semantico en lin %d: Variable no inicializada.\n", numero_linea);
			return -1;
			...
			if(lectura->tipo != ENTERO || $3.tipo != ENTERO) {
				fprintf(yyout,"****Error semántico en lin %d: se esperaba una expresión o variable de tipo entero.\n", numero_linea);
				return -1;	
			}
			...
			if(lectura->clase == VECTOR){
				...
				incrementar_array(yyout, $1.lexema, $3.es_direccion, lectura->adicional1, num_incrementos); 
				num_incrementos++;
			}
			else{
				if(lectura->tipo != $3.tipo){
				...
				if(UsoGlobal($1.lexema) == NULL){
					...
					incrementar(yyout, $1.lexema, $3.es_direccion, 1);
				}
				else{
					incrementar(yyout, $1.lexema, $3.es_direccion, 0);
					fprintf(yyout,";R115:\t<asignacion> ::= <identificador> += <exp>\n");
				}
			}
		}
	}
...
/****************************************Examen 2015***********************************************/
init_vector: TOK_INIT TOK_IDENTIFICADOR TOK_LLAVEIZQUIERDA lista_expresiones_init TOK_LLAVEDERECHA {
		...
		if(lectura == NULL){
			...
			if(num_init > lectura->adicional1){
				fprintf(yyout,"****Error semantico en lin %d: Lista de inicialización de longitud incorrecta.\n", numero_linea);
				return -1;
			}

			if(lectura->clase != VECTOR){
				fprintf(yyout, "****Error semantico en lin %d: Intento de inicializacion de una variable que no es de tipo vector.\n", numero_linea);
				return -1;
			}

			init_array(yyout, $2.lexema, lectura->adicional1, num_init, num_inits);
			num_inits++;
			num_init = 0;
			fprintf(yyout,";RINIT:\t<init_vector> ::= init %s { <lista_expresiones_init> } ;\n", $2.lexema);
		}
	}
;

lista_expresiones_init: exp_init resto_lista_expresiones_init {
		num_init++;
		fprintf(yyout,";RINIT:\t<lista_expresiones_init> ::= <exp_init> <resto_lista_expresiones_init> \n");
	} 
	| /* VACIO */ {
		fprintf(yyout,";RINIT:\t<lista_expresiones_init> ::=  \n");
	}
;

resto_lista_expresiones_init: TOK_PUNTOYCOMA exp_init resto_lista_expresiones_init {
		num_init++;
		fprintf(yyout,";RINIT:\t<resto_lista_expresiones_init> ::= ; <exp_init> <resto_lista_expresiones_init> \n");
	} 
	| /* VACIO */ {
		fprintf(yyout,";RINIT:\t<resto_lista_expresiones_init> ::=  \n");
	} 
;

exp_init: exp {
		if($1.es_direccion == 1){
			direccion_a_valor_init(yyout);
		}
		fprintf(yyout,";RINIT:\t<exp_init> ::= <exp>\n");
	}
;
/**************************************************************************************************/
...
condicional: if_exp_sentencias TOK_LLAVEDERECHA {
	...
	/* Examen 2014 */
	| compare_with_exp TOK_LLAVEIZQUIERDA less_sentencias equal_sentencias greater_sentencias TOK_LLAVEDERECHA {
		fprintf(yyout,";R109:\t<condicional> ::= <compare_with_exp> { <less_sentencias> <equal_sentencias> <greater_sentencias> }\n");
	}
	/***************/
;

/****************************************Examen 2014***********************************************/
/* Regla para la expresion de declaracion de un compare exp with exp */
compare_with_exp: TOK_COMPARE exp TOK_WITH exp {
		if($2.tipo != ENTERO || $4.tipo != ENTERO){
			fprintf(yyout,"**** Error semántico en lin %d: se esperaba una expresión de tipo entero.\n", numero_linea);
			return -1;
		}

		num_condicionales++;
		compareLess(yyout, $2.es_direccion, $4.es_direccion, num_condicionales);
		fprintf(yyout,";R109:\t<compare_with_exp> ::= compare ( <exp> ) with ( <exp> )\n");	
	}
;

/* Regla del bloque less */
less_sentencias: TOK_LESS sentencias {
		compareEqual(yyout, num_condicionales);
		fprintf(yyout,";R110:\t<less_sentencias> ::= less <sentencias>\n");	
	}
;

/* Regla del bloque equal */
equal_sentencias: TOK_EQUAL sentencias {
		compareGreater(yyout, num_condicionales);
		fprintf(yyout,";R111:\t<equal_sentencias> ::= equal <sentencias>\n");	
	}
;

/* Regla del bloque greater */
greater_sentencias: TOK_GREATER sentencias {
		compareWithFin(yyout, num_condicionales); 
		fprintf(yyout,";R112:\t<greater_sentencias> ::= greater <sentencias>\n");	
	}
;
/***************************************************************************************************/
...
exp: exp TOK_MAS exp {
	...
	| exp TOK_MODULO exp {
		...
		$$.tipo = ENTERO;		
		modulo(yyout, $1.es_direccion, $3.es_direccion);
		$$.es_direccion = 0;
		fprintf(yyout,";R113:\t<exp> ::= <exp> modulo <exp>\n");
	}
	/***************/
	/* Examen 2011 */
	| exp TOK_POTENCIA exp {
		...
		$$.tipo = ENTERO;	
		potencia(yyout, $1.es_direccion, $3.es_direccion, num_potencias);
		num_potencias++;	
		$$.es_direccion = 0;
		fprintf(yyout,";R114:\t<exp> ::= <exp> modulo <exp>\n");
	}
	/***************/
	/* Examen 2012 */
	| TOK_LONGITUD TOK_PARENTESISIZQUIERDO TOK_IDENTIFICADOR TOK_PARENTESISDERECHO {
		lectura = UsoLocal($3.lexema);
		if(lectura == NULL) {
			fprintf(yyout, "****Error semantico en lin %d: Acceso a variable no declarada (%s).\n", numero_linea, $3.lexema);
			return -1;
		}

		if(lectura->clase != VECTOR){
			fprintf(yyout, "****Error semantico en lin %d: La variable no es clase vector.\n", numero_linea);
			return -1;	
		}
		...
		tamanio_array(yyout, lectura->adicional1);
		fprintf(yyout,";R80:\t<exp> ::= long ( %s )\n", $3.lexema);
	}
	/***************/
;

...
/********************************** Examen 2015 **************************************/
void init_array(FILE* fpasm, char* vector, int tamano, int num_valores, int etiqueta) {
  fprintf(fpasm, "\tmov dword ecx, %d\n", num_valores); 
  fprintf(fpasm, "\tcmp ecx, 0\n");
  fprintf(fpasm, "\tje _todo_default_init%d\n", etiqueta); /* VEMOS SI HAY VALORES O TODO DEFAULT */
  
  fprintf(fpasm, "\tmov dword ecx, %d\n", num_valores-1); /* VALORES EN PILA ECX */
  fprintf(fpasm, "_init_vector%d:\n", etiqueta);
  fprintf(fpasm, "\tpop dword edx\n"); /* VALOR EN PILA EDX */
	fprintf(fpasm, "\tmov dword eax, _%s\n", vector);
	fprintf(fpasm, "\tlea eax, [eax + ecx*4]\n");  /* DIRECCION POS VECTOR EAX */
  fprintf(fpasm, "\tmov dword [eax], edx\n"); /* ASIGNAMOS VALOR PILA */
  fprintf(fpasm, "\tdec ecx\n");
  fprintf(fpasm, "\tcmp ecx, -1\n");
  fprintf(fpasm, "\tjne _init_vector%d\n", etiqueta); /* VUELTA A EMPEZAR SI QUEDAN VALORES PILA */

  fprintf(fpasm, "_todo_default_init%d:", etiqueta);
  fprintf(fpasm, "\tmov dword ebx, %d\n", num_valores-1); /* VALORES EN PILA EBX */
  fprintf(fpasm, "\tmov dword ecx, %d\n", tamano-1); /* TAMANIO ARRAY ECX */
  fprintf(fpasm, "\tsub ecx, ebx\n"); /* NUMERO ITERACIONES ECX-EBX */
  fprintf(fpasm, "\tcmp ecx, 0\n");
  fprintf(fpasm, "\tje _fin_init_vector%d\n", etiqueta); /* SI NO QUEDAN VALORES SIN VALOR SALTA A FIN */
  
  fprintf(fpasm, "_default_init%d:\n", etiqueta); /* ASIGNACION POR DEFECTO */
  fprintf(fpasm, "\tmov dword eax, _%s\n", vector);
  fprintf(fpasm, "\tmov dword edx, ebx\n");
  fprintf(fpasm, "\tadd edx, ecx\n"); /* POSICION VECTOR EDX */
  fprintf(fpasm, "\tlea eax, [eax + edx*4]\n");  /* DIRECCION POS VECTOR EAX */
  fprintf(fpasm, "\tmov dword [eax], 0\n"); /* ASIGNAMOS 0 */
  fprintf(fpasm, "\tdec ecx\n");
  fprintf(fpasm, "\tcmp ecx, 0\n");
  fprintf(fpasm, "\tjne _default_init%d\n", etiqueta); /* VUELTA A EMPEZAR SI QUEDAN VALORES */
  fprintf(fpasm, "_fin_init_vector%d:\n", etiqueta);
}

void direccion_a_valor_init(FILE* fpasm) {
  fprintf(fpasm, "\tpop dword eax\n");
  fprintf(fpasm, "\tmov dword eax, [eax]\n");
  fprintf(fpasm, "\tpush dword eax\n");
}
...
/********************************** Examen 2014 **************************************/
void compareWithFin(FILE * fpasm, int etiqueta) {
  fprintf(fpasm, "\t_finCompare%d:\n", etiqueta);
}

void compareLess(FILE * fpasm, int es_variable1, int es_variable2, int etiqueta) {
  fprintf(fpasm, "\tpop dword edx\n");
  if (es_variable2 == 1){
        fprintf(fpasm, "\tmov dword edx, [edx]\n");
  }
  ...
  fprintf(fpasm, "\tcmp eax, edx\n");
  fprintf(fpasm, "\tjg _greaterCompare%d\n", etiqueta);
  fprintf(fpasm, "\tje _equalCompare%d\n", etiqueta);
}

void compareEqual(FILE * fpasm, int etiqueta) {
  fprintf(fpasm, "\tjmp _finCompare%d\n", etiqueta);
  fprintf(fpasm, "_equalCompare%d:\n", etiqueta);
}

void compareGreater(FILE * fpasm, int etiqueta) {
  fprintf(fpasm, "\tjmp _finCompare%d\n", etiqueta);
  fprintf(fpasm, "_greaterCompare%d:\n", etiqueta);
}
/*************************************************************************************/
...
void incrementar(FILE* fpasm, char* nombre, int es_variable, int es_funcion) {
  if(es_funcion == 1){
    fprintf(fpasm, "\tpop dword ebx\n");
    fprintf(fpasm, "\tpop dword edx\n");
    if (es_variable == 1){
...
void incrementar_array(FILE* fpasm, char* vector, int es_inmediato, int tamano, int etiqueta) {
	fprintf(fpasm, "\tpop dword ebx\n");
	if (es_inmediato == 1) {
		fprintf(fpasm, "\tmov dword ebx, [ebx]\n");
	}
  fprintf(fpasm, "\tmov dword ecx, %d\n", tamano);
  fprintf(fpasm, "_incremento_vector%d:\n", etiqueta);
	fprintf(fpasm, "\tmov dword edx, _%s\n", vector);
	fprintf(fpasm, "\tlea eax, [edx + ecx*4]\n");
  fprintf(fpasm, "\tmov dword eax, [eax]\n");
  fprintf(fpasm, "\tadd eax, ebx\n");
	fprintf(fpasm, "\tmov dword edx, _%s\n", vector);
	fprintf(fpasm, "\tlea edx, [edx + ecx*4]\n");
  fprintf(fpasm, "\tmov dword [edx], eax\n");
  fprintf(fpasm, "\tdec ecx\n");
  fprintf(fpasm, "\tcmp ecx, -1\n");
  fprintf(fpasm, "\tjne _incremento_vector%d\n", etiqueta);
}
/*************************************************************************************/

/********************************** Examen 2013 **************************************/
void potencia(FILE* fpasm, int es_variable_1, int es_variable_2, int num_potencias) {
  fprintf(fpasm, "\tpop dword edx\n");
  if (es_variable_2 == 1){
    fprintf(fpasm, "\tmov dword edx, [edx]\n");
  }
  ...
  fprintf(fpasm, "\tmov dword ebx, eax\n");
  fprintf(fpasm, "_potencia%d:\n", num_potencias);
  fprintf(fpasm, "\tdec ecx\n");
  fprintf(fpasm, "\tcmp ecx, 0\n");
  fprintf(fpasm, "\tje _fin_potencia%d\n", num_potencias);
  fprintf(fpasm, "\tmov dword edx, ebx\n");
  fprintf(fpasm, "\timul eax, edx\n");
  fprintf(fpasm, "\tjmp _potencia%d\n", num_potencias);
  fprintf(fpasm, "_fin_potencia%d:\n", num_potencias);
  fprintf(fpasm, "\tpush dword eax\n");
}
/*************************************************************************************/

/********************************** Examen 2012 **************************************/
void tamanio_array(FILE* fpasm, int valor){
